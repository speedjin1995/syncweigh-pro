...
<?php
    if (!hasModulePermission('Master Data', 'Reason', ['view', 'create', 'edit'])){
        header('Location: no-permission.php');
        exit;
    }
?>
<head>
    <title>Reasons | PWS - Weighing System</title>
                                        ...
                                        <div class="modal-dialog modal-dialog-scrollable modal-lg">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="exampleModalScrollableTitle">Add New Reason</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                                                    </button>
                                                </div>
                                                <div class="modal-body">
                                                    <form role="form" id="reasonForm" class="needs-validation" novalidate autocomplete="off">
                                            ...
                                            <div class="modal-content">
                                                <form role="form" id="uploadForm">
                                                    ...
                                                    <div class="modal-footer justify-content-between bg-gray-dark color-palette">
                                                        ...
                                                        <button type="button" class="btn btn-danger" id="uploadReason">Save changes</button>
                                                    </div>
                                                </form>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row">
                                            <div class="col-lg-12">
                                                <div class="card">
                                                    <div class="card-header">
                                                        <div class="d-flex justify-content-between">
                                                            ...
                                                            <div class="flex-shrink-0">
                                                                ...
                                                                <?php if(hasModulePermission('Master Data', 'Reason', ['cancelled'])): ?>
                                                                ...
                                                                <?php if(hasModulePermission('Master Data', 'Reason', ['create'])): ?>
                                                                <button type="button" id="addReason" class="btn btn-danger waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#addModal">
                                                    ...
                                                    <div class="card-body">
                                                        <table id="reasonTable" class="table table-bordered nowrap table-striped align-middle" style="width:100%">
                                                            <thead>
                                                                <tr>
...
$(function () {
    $('#selectAllCheckbox').on('change', function() {
        var checkboxes = $('#reasonTable tbody input[type="checkbox"]');
        checkboxes.prop('checked', $(this).prop('checked')).trigger('change');
    });

    table = $("#reasonTable").DataTable({
        "responsive": true,
        "autoWidth": false,
        'processing': true,
        'serverSide': true,
            ...
            { 
                ...
                render: function ( data, type, row ) {
                    ...
                    if (row.status == 'Inactive') {
                        if (isSADMIN || (permissions['Master Data'] && permissions['Master Data']['Reason'] && permissions['Master Data']['Reason'].includes('reactivate'))){
                            buttons += `
                        ...
                        if (isSADMIN || (permissions['Master Data'] && permissions['Master Data']['Reason'] && ['edit', 'cancelled'].some(p => permissions['Master Data']['Reason'].includes(p)))) {
                            buttons += `
                                <div class="dropdown d-inline-block">
                                    <button class="btn btn-soft-secondary btn-sm dropdown" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                        <i class="ri-more-fill align-middle"></i>
                                    ...
                                    if (isSADMIN || (permissions['Master Data'] && permissions['Master Data']['Reason'] && permissions['Master Data']['Reason'].includes('edit'))){
                                        buttons += `
                                            <li>
                                    ...
                                    if (isSADMIN || (permissions['Master Data'] && permissions['Master Data']['Reason'] && permissions['Master Data']['Reason'].includes('cancelled'))){
                                        buttons += `
                                            <li>
    ...
    $('#submitReason').on('click', function(){
        if($('#reasonForm').valid()){
            $('#spinnerLoading').show();
            $.post('php/reason.php', $('#reasonForm').serialize(), function(data){
    ...
    $('#uploadReason').on('click', function(){
        $('#spinnerLoading').show();
    ...
    $('#exportExcel').on('click', function(){
        window.open("php/exportMasterData.php?selectedValue=Reason");
    });
    ...
    $('#multiDeactivate').on('click', function () {
        ...
        $("#reasonTable tbody input[type='checkbox']").each(function () {
            if (this.checked) {
...
function edit(id){
    ...
    $.post('php/getReason.php', {userID: id}, function(data)
    {
        ...
        if(obj.status === 'success'){
            ...
            $('#addModal').find('#reason').val(obj.message.reason);
...
function reactivate(id) {
  if (confirm('Do you want to reactivate this item?')) {
    $('#spinnerLoading').show();
    $.post('php/reactivateMasterData.php', {userID: id, type: "Reason"}, function(data){
        var obj = JSON.parse(data);
