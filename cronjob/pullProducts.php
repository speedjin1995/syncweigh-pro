...
if (!empty($data['data'])) {
    ...
    foreach ($agents as $agent) {
        if (!empty($agent['conversions']) && is_array($agent['conversions']) && isset($agent['UOM']) && !empty($agent['UOM'])) {
            ...
            $checkQuery = "SELECT COUNT(*) AS count FROM Product WHERE product_code = ?";
            ...
            if ($result['count'] > 0) {
                // Update if exists
                $updateQuery = "UPDATE Product SET name = ?, description = ?, basic_uom = ?, modified_by = ? WHERE product_code = ?";
                ...
                $insertQuery = "INSERT INTO Product (product_code, name, description, basic_uom, created_by, modified_by) VALUES (?, ?, ?, ?, ?, ?)";
            ...
            // Just to query the id
            $getIdQuery = "SELECT id FROM Product WHERE product_code = ?";
            ...
            $deleteStmt = $db->prepare("DELETE FROM Product_UOM WHERE product_id = ?");
            ...
            foreach ($agent['conversions'] as $conv) {
                ...
                if ($unitRow = $unitResult->fetch_assoc()) {
                    ...
                    // Step 2: Insert into Product_UOM
                    $productStmt = $db->prepare("INSERT INTO Product_UOM (product_id, unit_id, rate) VALUES (?, ?, ?)");
