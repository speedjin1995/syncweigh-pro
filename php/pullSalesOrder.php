...
$url = rtrim($config[$companyKey], '/') . "/sales_orders?start_date=$startDate&end_date=$endDate";
...
if (!empty($data['data'])) {
    ...
    foreach ($agents as $rows) {
        ...
        $BatchDrum = null;
        $BasePlantCode = $PlantCode;
        $PlantName = '';
        if (!empty($PlantCode)) {
            // Match optional -B or -D at the end
            if (preg_match('/^(.*?)-(B|D)$/i', $PlantCode, $matches)) {
                $BasePlantCode = $matches[1];   // ABC123
