...
, $_POST['manualWeight'], $_POST['plantCode'], $_POST['plant'], $_POST['exDel'], $_POST['loadDrum'])) {
    ...
    if(isset($_POST['basicUOM']) && $_POST['basicUOM'] != null && $_POST['basicUOM'] != ''){
        if ($transactionStatus == 'Sales'){
            $convertedOrderWeight = $_POST['basicUOM'];
            $convertedSupplierWeight = 0;
        }else{
            $convertedSupplierWeight = $_POST['basicUOM'];	
            $convertedOrderWeight = 0;
        }
    }
    else{
        $convertedOrderWeight = 0;
    ...
    if(isset($_POST['basicUOMUnit']) && $_POST['basicUOMUnit'] != null && $_POST['basicUOMUnit'] != ''){
        if ($transactionStatus == 'Sales'){
            $convertedOrderWeightUnit = $_POST['basicUOMUnit'];
            $convertedSupplierWeightUnit = null;
        }else{
            $convertedOrderWeightUnit = null;
            $convertedSupplierWeightUnit = $_POST['basicUOMUnit'];
        }
    }
    else{
        $convertedOrderWeightUnit = null;
    ...
    if(isset($_POST['convertedNettWeight']) && $_POST['convertedNettWeight'] != null && $_POST['convertedNettWeight'] != ''){
        $convertedNettWeight = $_POST['convertedNettWeight'];
    }
    else{
        $convertedNettWeight = 0;
    }
    ...
    if(! empty($weightId)){
        ...
        product_code=?, product_name=?, ex_del=?, raw_mat_code=?, raw_mat_name=?, site_name=?, site_code=?, container_no=?, invoice_no=?, purchase_order=?, delivery_no=?, transporter_code=?, transporter=?, destination_code=?, destination=?, remarks=?, gross_weight1=?, gross_weight1_date=?, tare_weight1=?, tare_weight1_date=?, nett_weight1=?, converted_nett_weight1=?,
        ...
        {
            $update_stmt->bind_param('sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss', $transactionId, $transactionStatus, $weightType, $customerType, $transactionDate, $vehiclePlateNo1, $vehiclePlateNo2, $supplierWeight, $convertedSupplierWeight, $convertedSupplierWeightUnit, $orderWeight, $convertedOrderWeight, $convertedOrderWeightUnit, $customerCode, $customerName,
            ...
            $grossIncoming, $grossIncomingDate, $tareOutgoing, $tareOutgoingDate, $nettWeight, $convertedNettWeight, $grossIncoming2, $grossIncomingDate2, $tareOutgoing2, $tareOutgoingDate2, $nettWeight2, $reduceWeight, $finalWeight, $weightDifference,
        ...
        product_description, unit_price, sub_total, sst, total_price, is_approved, approved_reason, plant_code, plant_name, agent_code, agent_name, load_drum, no_of_drum) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
            $insert_stmt->bind_param('ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss', $transactionId, $transactionStatus, $weightType, $customerType, $transactionDate, $vehiclePlateNo1, $vehiclePlateNo2, $supplierWeight, $convertedSupplierWeight, $convertedSupplierWeightUnit, $orderWeight, $convertedOrderWeight, $convertedOrderWeightUnit, $customerCode, $customerName,
            $supplierCode, $supplierName, $productCode, $productName, $exDel, $rawMaterialCode, $rawMaterialName, $siteCode, $siteName, $containerNo, $invoiceNo, $purchaseOrder, $deliveryNo, $transporterCode, $transporter, $destinationCode, $destination, $otherRemarks,
            $grossIncoming, $grossIncomingDate, $tareOutgoing, $tareOutgoingDate, $nettWeight, $convertedNettWeight, $grossIncoming2, $grossIncomingDate2, $tareOutgoing2, $tareOutgoingDate2, $nettWeight2, $reduceWeight, $finalWeight, $weightDifference,
