        ...
        if($query->num_rows > 0){ 
            ...
            while($row = $query->fetch_assoc()){ 
                ...
                if($row['order_no'] != null && $row['order_no'] != ''){
                    ...
                    $weightQuery = "SELECT SUM(nett_weight1) AS total_weight FROM Weight WHERE purchase_order = '$customerPONo' AND product_code = '$productCode' AND customer_code = '$customerCode' AND status = '0' AND transaction_status = 'Sales' AND is_cancel <> 'Y' ORDER BY id ASC";
        ...
        if($query->num_rows > 0){ 
            ...
            while($row = $query->fetch_assoc()){ 
                ...
                if($row['po_no'] != null && $row['po_no'] != ''){
                    ...
                    $weightQuery = "SELECT SUM(supplier_weight) AS total_weight FROM Weight WHERE purchase_order = '$poNo' AND raw_mat_code = '$rawMatCode' AND supplier_code = '$supplierCode' AND status = '0' AND transaction_status = 'Purchase' AND is_cancel <> 'Y' ORDER BY id ASC";
                    $weightRecords = mysqli_query($db, $weightQuery);
