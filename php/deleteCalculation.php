...
if(isset($_POST['userID'])){
	...
	if ($type == 'MULTI'){
		...
		if ($stmt2 = $db->prepare("UPDATE Calculations SET deleted=? WHERE id IN ($ids)")) {
		...
		if ($stmt2 = $db->prepare("UPDATE Calculations SET deleted=? WHERE id=?")) {
			$stmt2->bind_param('ss', $del , $id);
			
			if($stmt2->execute()){
				if ($stmt3 = $db->prepare("UPDATE Calculation_Value SET deleted=? WHERE calculation_id=?")) {
