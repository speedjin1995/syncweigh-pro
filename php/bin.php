...
if (isset($_POST['binCode'])) {
    ...
    if (empty($_POST["binCode"])) {
        $binCode = null;
    } else {
        $binCode = trim($_POST["binCode"]);
    }
    ...
    if (empty($_POST["binName"])) {
        $binName = null;
    } else {
        $binName = trim($_POST["binName"]);
    }
    ...
    if(! empty($binId))
    {
        ...
        $action = "2";
        if ($update_stmt = $db->prepare("UPDATE Bin SET bin_code=?, name=?, description=? , created_by=?, modified_by=? WHERE id=?")) 
        {
            $update_stmt->bind_param('ssssss', $binCode, $binName, $description, $username, $username, $binId);
    ...
    {
        $action = "1";
        if ($insert_stmt = $db->prepare("INSERT INTO Bin (bin_code, name, description, created_by, modified_by) VALUES (?, ?, ?, ?, ?)")) {
            $insert_stmt->bind_param('sssss', $binCode, $binName, $description, $username, $username);
