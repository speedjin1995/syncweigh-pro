...
if (!empty($data['data'])) {
    ...
    foreach ($agents as $agent) {
        ...
        if ($result['count'] > 0) {
            // Update if exists
            $updateQuery = "UPDATE Product SET name = ?, description = ?, modified_by = ? WHERE product_code = ?";
            ...
            $insertQuery = "INSERT INTO Product (product_code, name, description, created_by, modified_by) VALUES (?, ?, ?, ?, ?)";
