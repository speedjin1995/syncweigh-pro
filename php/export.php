...
$fields = array('TRANSACTION ID', 'TRANSACTION STATUS', 'WEIGHT TYPE', 'TRANSACTION DATE', 'LORRY NO.', 'CUSTOMER CODE', 'CUSTOMER NAME', 
    ...
    'DO NO.', 'GROSS WEIGHT', 'TARE WEIGHT', 'NET WEIGHT', 'IN TIME', 'OUT TIME', 'MANUAL', 'CANCELLED', 'PLANT CODE', 'PLANT NAME', 'WEIGHTED BY'); 
    ...
    while($row = $query->fetch_assoc()){ 
        if($_GET["file"] == 'weight'){
            if($row['product_code'] != '501A-011'){
                ...
                $row['tare_weight1_date'], $row['manual_weight'], $row['is_cancel'], $row['plant_code'], $row['plant_name'], $row['created_by']);
            }
        }
        else{
